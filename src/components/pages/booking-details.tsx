...
import { Calendar, Clock, MapPin, CreditCard, ArrowLeft, ExternalLink } from 'lucide-react';
...
import { getPaymentByBookingId, openPaymentInNewTab, type PaymentStatus, type PaymentResponse } from "@/lib/payment";
...
export const BookingDetails: React.FC = () => {
  ...
  useEffect(() => {
    const loadBookingDetails = async () => {
      if (!bookingId) {
        setState(prev => ({ ...prev, error: 'Booking ID is required', loading: false }));
        return;
      }
  ...
  const handlePayNow = () => {
    ...
    const pendingPayment = state.payments?.find(payment => payment.payment_status === 'PENDING');
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{field?.name || 'Loading...'}</span>
                  </div>
                  <div className="text-sm text-gray-600">
                    Location: {location?.name || 'Location information unavailable'}
                  </div>
                  {field?.description && (
                    <div className="text-sm text-gray-600">
                      {field.description}
            ...
            <div className="border-t pt-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">Total Amount: {formatPrice(booking.total_price)}</span>
                </div>
                    ...
                    {state.payments.map((payment) => (
                      <div key={payment.id} className="border rounded-lg p-4">
                        ...
                        <div className="text-sm text-gray-600 space-y-1">
                          ...
                          <div>Created: {format(new Date(payment.created_at), 'PPp')}</div>
                          {payment.paid_at && (
                            <div>Paid: {format(new Date(payment.paid_at), 'PPp')}</div>
                          )}
                        </div>
                      </div>
                    ))}
