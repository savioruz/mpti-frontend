...
function AnalyticsPage() {
  ...
  const generateLocationRevenueData = (
    locations: any[],
    payments: any[],
    bookings: any[],
    fields: any[],
    ...
    return locations.slice(0, 5).map((location) => {
      ...
      const locationFields = fields.filter(
        (field) => field.location_id === location.id,
      );
  ...
  const generatePaymentMethodData = (payments: any[]) => {
    ...
    return methods.map((method) => ({
      ...
      count:
        payments.filter((p) => p.payment_method?.includes(method.toLowerCase()))
